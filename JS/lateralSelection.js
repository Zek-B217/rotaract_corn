...
function lateralAnimation(animationSeconds){
    let animationCycles = FPS * animationSeconds;
    let ctr = 0;
    let opacityCyclicIncrease = CSS_VARIABLES.getPropertyValue("--obscurerFinalOpacity") / animationCycles; //Quanto aumentare l'opacità a ogni ciclo
    let rightDistanceCyclicIncrease = (
            parseFloat(CSS_VARIABLES.getPropertyValue("--obscurerFinalOpacity")) - 
    ...
    let currentRightDistanceValue = parseFloat(CSS_VARIABLES.getPropertyValue("--lateralSelectionCurrentPosition"));; //Valore right corrente
    ...
    function frame(){
        if (ctr < animationCycles){
            ...
            currentOpacityValue += opacityCyclicIncrease;
            ROOT.style.setProperty("--obscurerCurrentOpacity", currentOpacityValue);

            //Animazione posizione
            currentRightDistanceValue += rightDistanceCyclicIncrease;
            ROOT.style.setProperty("--lateralSelectionCurrentPosition", currentRightDistanceValue + "vw");
